...
%N=wavread(fin,'size');
[y, Fs ]= audioread([fin,'.wav']);

N =length(y);
 
% [sig,noise_mat ]= noisePowProposed(y, 16000); 
 
% compare =  [sig,noise_mat ];
 
% audiowrite([fout 'niseproposed.wav'],compare,16000); 
 
 lambda_array = zeros(N,1);
 sig_array = zeros(N,1);
...
for l=1:Nframes
    ...
    if (~fnz_flag && abs(y(1))>zero_thres) ||  (fnz_flag && any(abs(y)>zero_thres))       % new version omlsa3
        ...
        % use proposed noise estimate instead
      if noise_mode == 2
            lambda_dav=  noisePowProposedSingal(Ya2,l);
        ...
        lambda_array((l-1)*M21+1:(l)*M21) = lambda_dav;
        sig_array((l-1)*M21+1:(l)*M21) = Ya2;
 ...
 compare2 = [sig_array ,lambda_array];

 audiowrite([fout 'noise_cohen.wav'],compare2,16000); 
