function [y,out]=gsc_dual_postfilterV2( y_time, b_time ,fs, fin)
...
Y_BUF = y_time;% 0.5*( Y_BUF1 + U_BUF1) ;
U_BUF = b_time;%Y_BUF1 - U_BUF1;
...
while(EndFrame < len)
    ...
    if (~fnz_flag && abs(y(1))>zero_thres) ||  (fnz_flag && any(abs(y)>zero_thres))       % new version omlsa3
        ...
        %% 2) add dual channel q_dual estimation routine
         ...
         PSI2(idx_1_2) = (OMEGA(idx_1_2) - OMEGA_LO) ./ max((OMEGA_HI - OMEGA_LO),1e-10);
         
         k1_psi = 9; k2_psi = 113;  PSI_0 = 0.25; gamma_0 = 4.6;
         
         PSI_global = mean(PSI(k1_psi:k2_psi));
         PSI_global2 = mean(PSI2(k1_psi:k2_psi));
         
         if(PSI_global ~= PSI_global2)
            fprintf("Not the same PSI\n"); 
         end
 ...
 audiowrite([fin '_d_omlsaV2y.wav'],OutputFix,Fs,'BitsPerSample',16); 
