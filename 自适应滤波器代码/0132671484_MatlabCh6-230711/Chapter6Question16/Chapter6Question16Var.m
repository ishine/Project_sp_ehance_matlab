...
clear
clc
numberOfDatapoints=500; % try 300 to get a better picture of the process
numberOfRuns=9000;       % increase this value to get smoothed out image

a=-0.99; %AR paramter
NoiseVariance=0.0187; % the variance of the system noise given in the problem
mu=0.1; %LMS learning rate parameter it will interest the reader to also
                        %try also 0.5 and 0.75

stream = RandStream('mt19937ar','Seed',7);  % seed the random number
...
g=zeros(numberOfRuns,numberOfDatapoints);  % allocate memory of squared averaged error
...
for k=1:numberOfRuns % Loop for performing the appropriate number of 
    ...
    g(k,:)=f;               % accumulate squared error of estimation
end
g=var(g)           % normalize the accumulated error to reflect
                            % the average error based on the number of
                            % monte Carlo simulations completed
%g=g(4,:)
i=[1:numberOfDatapoints];
plot(i,g) % plot the MSE vs iterations
title('Graph of variance vs iteration for Problem 11.5')
