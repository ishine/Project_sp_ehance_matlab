...
MSE_dB_values=zeros(length(iSNR_db_values),length(Q_values));
nu_dB_values=zeros(length(iSNR_db_values),length(Q_values));
for idxQ=1:length(Q_values)
    ...
    for idxS=1:length(iSNR_db_values)
        ...
        MSE_dB_values(idxS,idxQ)=10*log10( Rx(1,1)+h'*Ry*h-h'*Rx*i_i-i_i'*Rx*h );
        
        nu_dB_values(idxS,idxQ)=10*log10( (h-i_i)'*Rx*(h-i_i)/Rx(1,1) );
    end
end
