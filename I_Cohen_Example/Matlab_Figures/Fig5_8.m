...
iSNR_db_values=-5:15;  
Gain_dB_values=zeros(length(iSNR_db_values),length(alpha_values));
MSE_values=zeros(length(iSNR_db_values),length(alpha_values));
xi_n_dB_values=zeros(length(iSNR_db_values),length(alpha_values));
xi_d_dB_values=zeros(length(iSNR_db_values),length(alpha_values));
for idxM=1:length(alpha_values)
    alpha=alpha_values(idxM);
    ...
    for idxS=1:length(iSNR_db_values)
        ...
        h=zeros(M,length(f));
        for idxf=1:length(f)
            dv=(du(:,idxf)+alpha*[1;zeros(M-1,1)])/(1+alpha);
            C=[d(:,idxf) dv];
            phiY=phiV(:,:,idxf)+phiX1(idxf)*d(:,idxf)*d(:,idxf)';
            h(:,idxf)=inv(phiY)* C * inv(C' *inv(phiY)*C )*[1;0];
        end
        ...
        MSE_values(idxS,idxM)=10*log10(int_H2_phiV+int_Hm12_phiX);
