...
W1_dB_values=zeros(length(f),length(delta_values));
W2_dB_values=zeros(length(f),length(delta_values));
W3_dB_values=zeros(length(f),length(delta_values));
W4_dB_values=zeros(length(f),length(delta_values));
...
for idxD=1:length(delta_values)
    ...
    for idx_f=1:length(f)
        ...
        alpha11=0; % dipole
        h=1/(1-exp(1i*2*pi*fk*delta/c*(1-alpha11)))*[1; -exp(-1i*2*pi*fk*delta/c*alpha11)];
        W1_dB_values(idx_f,idxD)=10*log10(abs(h'*d)^2/(h'*h));
        alpha11=-1; % cardioid
        h=1/(1-exp(1i*2*pi*fk*delta/c*(1-alpha11)))*[1; -exp(-1i*2*pi*fk*delta/c*alpha11)];
        W2_dB_values(idx_f,idxD)=10*log10(abs(h'*d)^2/(h'*h));
        alpha11=-1/3; % hypercardioid
        h=1/(1-exp(1i*2*pi*fk*delta/c*(1-alpha11)))*[1; -exp(-1i*2*pi*fk*delta/c*alpha11)];
        W3_dB_values(idx_f,idxD)=10*log10(abs(h'*d)^2/(h'*h));
        alpha11=(1-3^0.5)/(3-3^0.5); % supercardioid
        h=1/(1-exp(1i*2*pi*fk*delta/c*(1-alpha11)))*[1; -exp(-1i*2*pi*fk*delta/c*alpha11)];
        W4_dB_values(idx_f,idxD)=10*log10(abs(h'*d)^2/(h'*h));
    end
end
