...
mu=[0.5 1 2 5]';
gain_values=zeros(length(broadband_iSNR),length(mu));
MSE_values=zeros(length(broadband_iSNR),length(mu));
xi_n_values=zeros(length(broadband_iSNR),length(mu));
xi_d_values=zeros(length(broadband_iSNR),length(mu));

for k=1:length(mu)
    
    H=narrowband_iSNR./(mu(k)+narrowband_iSNR);
...
plot(broadband_iSNR_dB(1:2:end),gain_values(1:2:end,4),'-.c^','linewidth',linewd, 'MarkerSize',MarkerSize);
hold off
set(gca, 'Color', [1, 1, 1]); 
...
plot(broadband_iSNR_dB(1:2:end),MSE_values(1:2:end,4),'-.c^','linewidth',linewd, 'MarkerSize',MarkerSize);
hold off
set(gca, 'Color', [1, 1, 1]); 
set(gca, 'FontName', 'Times New Roman');
set(gca, 'FontSize', hcfontsize);
...
plot(broadband_iSNR_dB(1:2:end),xi_n_values(1:2:end,4),'-.c^','linewidth',linewd, 'MarkerSize',MarkerSize);
hold off
set(gca, 'Color', [1, 1, 1]); 
set(gca, 'FontName', 'Times New Roman');
set(gca, 'FontSize', hcfontsize);
...
plot(broadband_iSNR_dB(1:2:end),xi_d_values(1:2:end,4),'-.c^','linewidth',linewd, 'MarkerSize',MarkerSize);
hold off
set(gca, 'Color', [1, 1, 1]); 
set(gca, 'FontName', 'Times New Roman');
set(gca, 'FontSize', hcfontsize);
