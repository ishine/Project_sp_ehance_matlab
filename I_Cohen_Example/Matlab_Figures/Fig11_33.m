...
for idxM=1:length(M_values)
    for idxD=1:length(Lh_vec)
        ...
        G_theta_2=zeros(M*Lh,L);
        for m=1:M
            Gm_theta_2=sinc(-P-Lh+1-ii+jj-fs*(m-1)*delta/c*cos(theta_2));
            G_theta_2(((m-1)*Lh+1):(m*Lh),:)=Gm_theta_2;
        end
        G_theta_3=zeros(M*Lh,L);
        for m=1:M
            Gm_theta_3=sinc(-P-Lh+1-ii+jj-fs*(m-1)*delta/c*cos(theta_3));
            G_theta_3(((m-1)*Lh+1):(m*Lh),:)=Gm_theta_3;
        end
        C=[G_theta_d G_theta_1 G_theta_2 G_theta_3];
        i_3=[i_ell; zeros(L,1); zeros(L,1); zeros(L,1)];
        h=C*pinv(C'*C+1e-5*eye(4*L))*i_3;
