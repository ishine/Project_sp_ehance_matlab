...
iSNR_db_values=-5:15;
Gain_dB_values=zeros(length(iSNR_db_values),length(Pl_values));
MSE_values=zeros(length(iSNR_db_values),length(Pl_values));
xi_n_dB_values=zeros(length(iSNR_db_values),length(Pl_values));
xi_d_dB_values=zeros(length(iSNR_db_values),length(Pl_values));

[t2,t1]=meshgrid(1:L,1:L);
for idxM=1:length(Pl_values)
    Pl=Pl_values(idxM);
