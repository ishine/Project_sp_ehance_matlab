function [y,out]=gsc_dual_postfilter(fin )
...
while(EndFrame < len)
    ...
    if (~fnz_flag && abs(y(1))>zero_thres) ||  (fnz_flag && any(abs(y)>zero_thres))       % new version omlsa3
        ...
        %% 2) add dual channel q_dual estimation routine
         ...
         idx_1_1  = OMEGA   <=  OMEGA_LO;         
         PSI(idx_1_1) = 0;         
         
         idx_1_2  =  OMEGA  <= OMEGA_HI  & OMEGA >OMEGA_LO;
         PSI(idx_1_2) = (OMEGA(idx_1_2) - OMEGA_LO) ./(OMEGA_HI - OMEGA_LO);
         
         k1_psi = 9; k2_psi = 113;  PSI_0 = 0; gamma_0 = 4.6;
