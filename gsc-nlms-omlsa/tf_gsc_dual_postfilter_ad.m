...
function [out_om]=tf_gsc_dual_postfilter_ad(y_frame_in , u_frame_in , framecnt,    arg1, arg2)
...
global broad_flag; global tone_flag; global nonstat;
  ...
  global Mno; global out_omlsa;    
  global SW; global  SWt; global SW_u; global SWt_u Ya2 Ua2   ;
...
%if(EndFrame < len)
    ...
    if (~fnz_flag && abs(y(1))>zero_thres) ||  (fnz_flag && any(abs(y)>zero_thres))       % new version omlsa3
     ...
     %% 2  Noise estimation using IMCRA     Ya2 Ua2: voice power    S  S_u: Smoothed voice power in freq and time  
        ...
        q   = q_estimate(eta, 1 , framecnt,lambda_dav_long, Fs );
        ...
        lambda_d  = alpha_d_hat .* lambda_d + (1- alpha_d_hat) .* Ya2;
        
        % ref noise
        q = q_estimate(eta_u,2, framecnt,lambda_dav_long_u , Fs);
        
        gamma=Ua2./max(lambda_d_u,1e-10);
        eta=alpha_eta*eta_2term_u+(1-alpha_eta)*max(gamma-1,0);
        eta=max(eta,eta_min);
        v=gamma.*eta./(1+eta);
        PH1=zeros(M21,1);
        idx=find(q<0.9);
        PH1(idx)=1./(1+q(idx)./(1-q(idx)).*(1+eta(idx)).*exp(-v(idx)));        
        alpha_d_hat = alpha_d  + (1-alpha_d)*PH1;        
        lambda_d_u = alpha_d_hat .* lambda_d_u + (1- alpha_d_hat) .* Ua2;        
        ...
        %% 3) add dual channel q_dual estimation routine
         ...
         LAMBDA_U = S_u ./ max(lambda_d_u, 1e-10);
          k1_psi = 9; k2_psi = 80;  PSI_0 = 0.2; gamma_0 = 4.6;
         ...
         q_hat = zeros(M21,1); gamma_s = ones(M21,1);
        ...
        % q_hat(idx) = max((gamma_0 - gamma_s(idx)) /(gamma_0 - 1), 1- PSI(idx)  );
          ...
          q_hat  = max((gamma_0 - gamma_s ) /(gamma_0 - 1), 1- PSI  );
          idx = gamma_s >1 & PSI_global > PSI_0;      
           q_hat(idx) = 0;
        %%
