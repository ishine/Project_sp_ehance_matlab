...
[Y_BUF,Fs]=audioread([fin,'.wav']);  % read size of input data, Fs and NBITS
N = length(y);
OutputFix = zeros(N,1, 'int16');
...
for l=1:Nframes
    ...
    if (~fnz_flag && abs(y(1))>zero_thres) ||  (fnz_flag && any(abs(y)>zero_thres))       % new version omlsa3
        ...
        gamma=Ya2./max(lambda_d,1e-10); %后验数据 posteriori SNRS  gmma = |Y(k,l)|^2 / lambda_d, here use 1e-10 incase zero divider 
        eta=alpha_eta*eta_2term+(1-alpha_eta)*max(gamma-1,0); % 先验数据 priori SNRS
        eta=max(eta,eta_min);
        v=gamma.*eta./(1+eta);
        ...
        gamma=Ya2./max(lambda_d,1e-10);                       % postier SNR eq.10 in paper
        eta=alpha_eta*eta_2term+(1-alpha_eta)*max(gamma-1,0); % proior SNR  eq.28  
        eta=max(eta,eta_min);
        v=gamma.*eta./(1+eta);                                % eq.10   
  ...
  %   X=[zeros(3,1); G(4:M21-1).*Y(4:M21-1); 0];
   ...
   X(1:M21) = Y(1:M21);
      ...
      %  x=Cwin^2*win.*real(ifft(Y));
        ...
        OutputFix((l-1) * Mno+1 : (l) * Mno  ) = out(1:Mno) .*2^15;
...
audiowrite([fin '_omlsanew.wav'],OutputFix,16000,'BitsPerSample',16); 
