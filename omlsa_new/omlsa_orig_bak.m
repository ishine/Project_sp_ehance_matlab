...
fin = 'F:/Work/2018/Beamforming/matlab/GSCLMS/voice/test1';
nin=nargin;
if nin<2
    fout=[fin,'_origomlsa_bak'];
end

% 1) Parameters of Short Time Fourier Analysis:
Fs_ref=16e3;		% 1.1) Reference Sampling frequency
M_ref=320;		% 1.2) Size of analysis window
...
%        y=fread(finID,M,'int16')/2^15;
 ...
 %   end
       ...
       y = y_all(init_frame:end_frame);
       init_frame = init_frame +  Mno;
       end_frame =  end_frame+Mno;
   ...
   x_all((l-1)*Mno+1: l*Mno) = out(1:Mno);
...
audiowrite([fout '.wav'],x_all,16000,'BitsPerSample',16); 
%close(waitHandle)
