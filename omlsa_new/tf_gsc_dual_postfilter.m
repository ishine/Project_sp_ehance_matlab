...
global broad_flag; global tone_flag; global nonstat  xi_m_dB;
 ...
 global l_fnz; global l_omlsa;
  ...
  global SW; global  SWt; global SW_u; global SWt_u xi xi_frame ;
...
%if(EndFrame < len)
    ...
    if (~fnz_flag && abs(y(1))>zero_thres) ||  (fnz_flag && any(abs(y)>zero_thres))       % new version omlsa3
     ...
     %% 2  Noise estimation using IMCRA     Ya2 Ua2: voice power    S  S_u: Smoothed voice power in freq and time  
      ...
      %    eta=alpha_eta*   eta_2term +(1-alpha_eta)*max(gamma-1,0); % eta=alpha_eta*   eta_2term_y+(1-alpha_eta)*max(gamma-1,0);
